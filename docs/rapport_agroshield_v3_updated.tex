...
Le benchmarking doit être lu avec rigueur. Il existe deux niveaux de comparaison qui ne doivent pas être mélangés. Le premier niveau est un benchmark d'architecture : il utilise des indicateurs publics comparables, notamment la précision Top-1 ImageNet, le nombre de paramètres et le coût de calcul en GFLOPS, afin de choisir des familles de modèles reconnues avant l'entraînement spécifique AgroShield \cite{torchvisionmodels}. Le second niveau est le benchmark expérimental interne : il compare uniquement les modèles réellement entraînés sur les mêmes données AgroShield, avec accuracy, précision, rappel, score F1, matrice de confusion et rejet hors domaine.

Les architectures retenues dans le tableau suivant ne sont pas choisies au hasard. Elles couvrent les familles les plus connues et pertinentes pour une tâche de classification d'images : VGG comme baseline historique, Inception comme CNN multi-échelle, DenseNet comme réseau à connexions denses, MobileNetV2 comme référence mobile, EfficientNet-B0 comme architecture légère optimisée, et ResNet-50 comme référence résiduelle robuste \cite{vgg,inceptionv3,densenet,mobilenetv2,efficientnet,resnet}. Ce tableau ne compare donc pas des F1-scores, car ces scores n'ont de sens que lorsque les modèles sont évalués sur le même dataset et le même split.
...
\hline
\textbf{Architecture reconnue} & \textbf{Top-1 ImageNet} & \textbf{Paramètres} & \textbf{GFLOPS} & \textbf{Décision pour AgroShield} \\
\hline
VGG16 & 71,59\,\% & 138,4 M & 15,47 & Baseline historique utile pour comprendre l'évolution des CNN, mais beaucoup trop lourde et peu efficace pour une boucle embarquée. \\
\hline
Inception-v3 & 77,29\,\% & 27,2 M & 5,71 & Architecture reconnue et multi-échelle, mais plus lourde et moins simple à déployer qu'un modèle léger sur Raspberry Pi. \\
\hline
DenseNet-121 & 74,43\,\% & 8,0 M & 2,83 & Modèle crédible grâce à la réutilisation des caractéristiques, mais moins avantageux qu'EfficientNet-B0 en rapport précision--coût. \\
\hline
MobileNetV2 & 72,15\,\% & 3,5 M & 0,30 & Très bon repère mobile ; cependant EfficientNet-B0 offre une précision publique nettement supérieure pour un coût encore compatible avec l'embarqué. \\
\hline
\textbf{EfficientNet-B0} & \textbf{77,69\,\%} & \textbf{5,3 M} & \textbf{0,39} & \textbf{Choisi comme modèle principal : bon compromis entre précision, taille, coût de calcul et préparation Raspberry Pi.} \\
\hline
\textbf{ResNet-50} & \textbf{80,86\,\%} & \textbf{25,6 M} & \textbf{4,09} & \textbf{Choisi comme modèle de référence : très reconnu, robuste, utile pour comparer et contrôler EfficientNet-B0 hors ligne.} \\
\hline
\end{tabular}
...
Ce benchmark valorise le choix effectué : EfficientNet-B0 est retenu car il offre un rapport précision--complexité favorable pour l'embarqué, tandis que ResNet-50 est conservé comme modèle de référence reconnu. AgroShield combine donc un modèle de terrain et un modèle de contrôle, au lieu de choisir uniquement le modèle public le plus lourd.

\begin{importantbox}{Pourquoi YOLO n'est pas dans le benchmark principal ?}
YOLO est une famille de modèles de détection d'objets : elle sert à localiser des objets ou lésions avec des boîtes, et s'évalue typiquement avec des métriques de détection comme mAP et IoU \cite{yolo}. Le problème traité ici est différent : AgroShield classe une image foliaire parmi 15 classes. YOLO deviendrait pertinent dans une évolution du projet si le dataset contenait des annotations de localisation des lésions. Sans boîtes annotées, le comparer directement à EfficientNet-B0 ou ResNet-50 serait méthodologiquement incorrect.
\end{importantbox}
...
\hline
\textbf{Modèle entraîné AgroShield} & \textbf{Accuracy semi-finale} & \textbf{Taille locale} & \textbf{Rôle} & \textbf{Décision d'ingénierie} \\
\hline
...
Les scores AgroShield ci-dessus sont les résultats semi-finaux déjà obtenus sur les deux modèles entraînés. Les métriques essentielles à présenter pour l'évaluation finale sont l'accuracy, la précision macro, le rappel macro, le F1 macro, le F1 pondéré, la matrice de confusion, la taille du modèle et la latence CPU. Ces métriques sont pertinentes uniquement si elles sont calculées sur le même split de test isolé. Le script \texttt{src/pipeline/08b\_benchmark\_modeles.py} calcule ces indicateurs sur EfficientNet-B0 et ResNet-50 avec le manifeste strict du test set.
...
\bibitem{vgg}
K. Simonyan et A. Zisserman, \textit{Very Deep Convolutional Networks for Large-Scale Image Recognition}, International Conference on Learning Representations, 2015. Disponible : \url{https://arxiv.org/abs/1409.1556}

\bibitem{inceptionv3}
C. Szegedy, V. Vanhoucke, S. Ioffe, J. Shlens et Z. Wojna, \textit{Rethinking the Inception Architecture for Computer Vision}, IEEE Conference on Computer Vision and Pattern Recognition, 2016. Disponible : \url{https://arxiv.org/abs/1512.00567}

\bibitem{densenet}
G. Huang, Z. Liu, L. van der Maaten et K. Q. Weinberger, \textit{Densely Connected Convolutional Networks}, IEEE Conference on Computer Vision and Pattern Recognition, 2017. Disponible : \url{https://arxiv.org/abs/1608.06993}

\bibitem{mobilenetv2}
M. Sandler, A. Howard, M. Zhu, A. Zhmoginov et L.-C. Chen, \textit{MobileNetV2: Inverted Residuals and Linear Bottlenecks}, IEEE Conference on Computer Vision and Pattern Recognition, 2018. Disponible : \url{https://arxiv.org/abs/1801.04381}

\bibitem{yolo}
J. Redmon, S. Divvala, R. Girshick et A. Farhadi, \textit{You Only Look Once: Unified, Real-Time Object Detection}, IEEE Conference on Computer Vision and Pattern Recognition, 2016. Disponible : \url{https://arxiv.org/abs/1506.02640}
