...
\section{Benchmarking multicritère des modèles}
Le choix entre EfficientNet-B0 et ResNet-50 ne doit pas se limiter à la précision globale. Dans AgroShield, le modèle retenu doit être performant, mais aussi suffisamment léger pour la Raspberry Pi, stable sur les classes rares et exploitable dans une boucle capteurs--caméra--action mécanique. Le benchmark est donc défini selon plusieurs critères : précision globale, F1-score macro, F1-score pondéré, matrice de confusion, taille du fichier modèle, nombre approximatif de paramètres et latence CPU moyenne.

\begin{table}[H]
\centering
\small
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|p{3.2cm}|C{2.3cm}|C{2.3cm}|p{6.2cm}|}
\hline
\textbf{Critère} & \textbf{EfficientNet-B0} & \textbf{ResNet-50} & \textbf{Interprétation projet} \\
\hline
Précision globale semi-finale & 99,42\,\% & 98,62\,\% & EfficientNet-B0 est légèrement supérieur sur le test semi-final. Ces valeurs doivent être recalculées avec le split strict avant conclusion définitive. \\
\hline
Taille du fichier \texttt{.pth} & 15,64 Mo & 90,09 Mo & EfficientNet-B0 est nettement plus adapté au transfert et au stockage embarqué. \\
\hline
Paramètres approximatifs & 5,3 M & 23,5 M & ResNet-50 est plus lourd ; il sert mieux comme modèle de comparaison hors ligne. \\
\hline
F1 macro / F1 pondéré & À recalculer par script & À recalculer par script & Les F1-scores sont essentiels pour vérifier que les classes rares ne sont pas masquées par l'accuracy globale \cite{brownleeimbalanced}. \\
\hline
Latence CPU Raspberry/PC & À mesurer & À mesurer & Le script \texttt{08b\_benchmark\_modeles.py} mesure la latence moyenne par image dans l'environnement cible. \\
\hline
Rôle recommandé & Déploiement prioritaire & Référence comparative & Le choix d'ingénierie privilégie EfficientNet-B0 pour le terrain, ResNet-50 pour contrôle et comparaison. \\
\hline
\end{tabular}
\caption{Benchmarking multicritère pour le choix du modèle}
\label{tab:benchmark_modeles}
\end{table}

Le dépôt contient le script \texttt{src/pipeline/08b\_benchmark\_modeles.py}, qui doit être exécuté après génération du manifeste strict de test. Il produit un fichier JSON et un fichier CSV avec les métriques nécessaires. Ainsi, le rapport ne s'appuie pas uniquement sur une figure de matrice de confusion : il fournit un protocole reproductible pour comparer les modèles selon les critères utiles au déploiement.
